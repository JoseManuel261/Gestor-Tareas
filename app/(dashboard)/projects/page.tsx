...
export default function ProjectsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-up">
            ...
            <div key={project.id}
              ...
              <div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-semibold text-sm leading-tight cursor-pointer hover:opacity-80 transition-opacity"
                    onClick={() => openEditProject(project)}
                    style={{ color: 'var(--text)' }}>{project.name}</h3>
      ...
      {showModal && (
        <Modal title={editingProjectId ? "Editar proyecto" : "Nuevo proyecto"} onClose={() => { setShowModal(false); setEditingProjectId(null); setForm({ name: '', description: '' }) }}>
          <form onSubmit={createProject} className="space-y-4">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? (editingProjectId ? 'Guardando...' : 'Creando...') : (editingProjectId ? 'Guardar cambios' : 'Crear proyecto')}
            </button>
          </form>
        </Modal>
      )}
